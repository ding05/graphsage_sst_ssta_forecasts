...
test_node_feats = node_feat_grid[:, 760 + window_size - lead_time + 1:]
...
#model, model_class = MultiGraphGAT(in_channels=graph_list[0].x[0].shape[0], hid_channels=30, out_channels=1, num_heads=4, num_graphs=len(train_graph_list)), 'GAT'
...
optimizer = torch.optim.RMSprop(model.parameters(), lr=learning_rate, alpha=0.9, weight_decay=0.0001, momentum=0.9)
...
torch.save({
            ...
            'loss': loss
            }, models_path + '_' + model_class + '_' + adj_filename[8:-4] + '_' + str(stop))
